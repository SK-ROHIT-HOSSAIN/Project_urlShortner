...
/*const shortUrl = async(req, res) => {
    try {
        ...
        if (Object.keys(longUrl).length == 0) return res.status(400).send({ status: false, msg: "enter long Url" });

        if (!isURL(longUrl)) return res.status(400).send({ status: false, msg: "invalid url" });

        if (!validator.isURL(longUrl)) return res.status(400).send({ status: false, msg: "enter long Url" });


        const cachedata = await GET_ASYNC(`${longUrl}`)
        if (cachedata) {
            return res.status(200).send({ status: true, msg: cachedata })
        }

        const dbData = await urlModel.findOne({ longUrl: longUrl }).select({ longUrl: 1, shortUrl: 1, urlCode: 1, _id: 0 });
        if (dbData) {
            return res.status(200).send({ status: true, msg: dbdata })
        }

        const urlCode = shortId.generate();
        const baseUrl = "http://localhost/3000";
        const shortId = baseUrl + "/" + urlCode;

        const savedata = { urlCode, baseUrl, shortId };

        const data = await urlModel.create(savedata);
